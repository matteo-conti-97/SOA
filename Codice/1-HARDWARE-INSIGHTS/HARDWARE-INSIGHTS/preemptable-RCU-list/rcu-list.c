...
void rcu_list_init(rcu_list * l){
	...
	pthread_spin_init(&l->write_lock,PTHREAD_PROCESS_PRIVATE);
        ...
        if ((pthread_create(&tid,NULL,house_keeper,(void*)(&the_list)) != 0)){//this thread can be activated vedi COMMENTO
...
/*COMMENTO Questo thread serve ad azzerare periodicamente i counter forzando un cambio di epoca senza
apportare aggiornamento, questo perchè se sulla struttura ho tanti lettori ma pochi aggiornamenti i counter
potrebbero andare in overflow*/
void * house_keeper(void * the_list){
